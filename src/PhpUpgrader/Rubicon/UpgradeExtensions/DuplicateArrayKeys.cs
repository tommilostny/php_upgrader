...
public static partial class DuplicateArrayKeys
{
    ...
    [GeneratedRegex(@"\$(cz_)?osetreni(_url)?\s?=\s?array\((""([^""]|\\""){0,9}""\s?=>\s?""([^""]|\\""){0,9}"",? ?)+\)", RegexOptions.ExplicitCapture, matchTimeoutMilliseconds: 3456)]
    private static partial Regex DupKeysArrayRegex();
    
    [GeneratedRegex(@"""([^""]|\\""){0,9}""\s?=>\s?""([^""]|\\""){0,9}""", RegexOptions.ExplicitCapture, matchTimeoutMilliseconds: 3456)]
