...
public sealed class UndefinedConstTests : UnitTestWithOutputBase
{
    ...
    public void UpdatesValid(string content)
    {
        //Arrange
    ...
    [InlineData("<?php \r\n                                    $stav_zbozi = $product['stav'];//najdi_v_db_2(\"dealers_data\",\"ean\",$krok3_ean,\"dealer_id\",$product['prodejna_id'],\"quantity\");\r\n                                    if($stav_zbozi == \"do 48 hodin\") {\r\n\t\t\t\t\t\t\t\t\t\techo \"<span style=\\\"color:#F86A04\\\">\".$stav_zbozi.\"</span>\";\r\n\t\t\t\t\t\t\t\t\t\techo '<input type=\"hidden\" name=\"do48h_'.$USER_KOSIK_POLOZKY[$product['product']][\"product_spec_id\"].'\" value=\"ESVO\" />';\r\n\t\t\t\t\t\t\t\t\t} elseif($stav_zbozi == \"na dotaz\") {\r\n\t\t\t\t\t\t\t\t\t\techo \"<span style=\\\"color:#F86A04\\\">\".$stav_zbozi.\"</span>\";\r\n\t\t\t\t\t\t\t\t\t\t//echo '<input type=\"hidden\" name=\"do48h_'.$USER_KOSIK_POLOZKY[$product['product']][\"product_spec_id\"].'\" value=\"ESVO\" />';\r\n\t\t\t\t\t\t\t\t\t} elseif ($stav_zbozi < 6) {\r\n\t\t\t\t\t\t\t\t\t\techo \"skladem \".$stav_zbozi.\" ks\";\r\n\t\t\t\t\t\t\t\t\t} else {\r\n\t\t\t\t\t\t\t\t\t\techo \"skladem >5 ks\";\r\n\t\t\t\t\t\t\t\t\t}\r\n                                    ?>")]
    public void DoesNotUpdateInvalid(string content)
    {
        //Arrange
        var file = new FileWrapper("file.php", content);
